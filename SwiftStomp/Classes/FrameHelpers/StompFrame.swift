...
struct StompFrame<T: RawRepresentable> where T.RawValue == String {
    ...
    init<X: Encodable>(name: T, headers: [String: String] = [:], encodableBody: X,
    ...
    mutating func deserialize(frame: String) throws {
        // Если в сообщении один символ переноса строки "\n", то отпределяем это как Heartbeat от сервера
        if frame == "\n" {
        ...
        while let line = lines.first, !line.isEmpty {
            ...
            headers[headerParts[0].trimmingCharacters(in: .whitespacesAndNewlines)] = headerParts[1]
